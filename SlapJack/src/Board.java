...
public class Board extends JFrame {
   ...
   public class CardsPanel extends JPanel{
      ...
      public void paintComponent(Graphics g){
         ...
         int h = getWidth()/2-71;
         g.drawLine(getWidth()/2, 10, getWidth()/2, 500);

         //displays the cpu card pile until it has no more cards
         int numCpuCards = cpu.getHand().size(); //get number of cpu cards
         if(numCpuCards > 0) {
            g.drawImage(back, h, 100, 143, 200, getContentPane()); //display the back of cpu card
         ...
         if(numDeckCards > 0) {
            int y = (getHeight() - 250)/2; //find where card is located on JPanel

            //displays the most recently flipped card
            g.drawImage(cards.get(cards.size() - 1).getImage(), h, y, 143, 200, getContentPane());
            Polygon deckShadow = new Polygon();
            deckShadow.addPoint(h+143, y);
            deckShadow.addPoint(h+143, y + 200);
         ...
         if(numPlayerCards > 0) {
            ...
            g.drawImage(back, h, y, 143, 200, getContentPane()); //display back of player card
